...
This phase ships the journal, the journal reader, and the alignment primitive. It does **not** ship detach, takeover, reconnection, or journal reaping. Nothing in this release drives a takeover: `RunRecord.cancelRequested`, `detachedSince`, and `sandboxKey` are still unpopulated. What a user can do today is run an agent through a journal, read that journal back from byte 0 on the same host, and replay a journal against a stored log without duplicating what is already there.
...
**Journal lifetime.** Reaching the sentinel means the run is terminal and the event log is now the run's record, so both journal files are deleted before `readJournalNdjson` finishes. The ordering is load-bearing and asserted: the follower is stopped before its input is removed, and the stderr sidecar is read for the error message before the deletion that destroys it. A stream that ends without a sentinel deletes nothing, since the run may be mid-flight and a successor may still need every byte. One case is not bounded: a run that reaches its sentinel while detached has no reader, so nothing observes the sentinel and that journal is not cleaned up yet.
