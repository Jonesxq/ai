 ...
 *
 * The `Sandbox` delegate is resolved LAZILY, on first use. Chat-only clients
 * whose schema omits the `Sandbox` model never trigger resolution (it only
 * runs when `withSandbox` uses the store).
 */
