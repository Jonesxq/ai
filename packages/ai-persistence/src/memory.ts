...
/**
 * In-process reference backend for the **chat** state stores plus the optional
 * generation media stores.
 *
 * Returns messages + runs + interrupts + metadata (the chat persistence shape)
 * widened with `artifacts` + `blobs`, so it satisfies `withPersistence` and the
 * byte-persisting path of `withGenerationPersistence` alike. Locks are not
 * included — use `InMemoryLockStore` + `withLocks` from `@tanstack/ai` when a
 * test or single-process app needs coordination.
 */
