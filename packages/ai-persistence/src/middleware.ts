...
/**
 * Generation entrypoint invalid when:
 * - `runs` is known-absent, or
 * - exactly one of `artifacts` / `blobs` is known-present (the byte path writes
 *   to both, so one without the other is always a misconfiguration).
 */
 ...
 *
 * Requires `stores.runs`. `stores.artifacts` + `stores.blobs` (as a pair) opt
 * into durable media bytes.
 *
 ...
 *
 * The artifact path below does **not** resolve this: it keys `ArtifactRecord`
 * on `ctx.runId ?? ctx.requestId` while the run record above is still keyed on
 * `ctx.requestId`, so a caller-supplied `runId` leaves `runs.get(runId)` empty
 * while `artifacts.list(runId)` returns rows. The two stores cannot be joined
 * until the job store lands.
 */
...
export function withGenerationPersistence(
  ...
  const { wantsArtifactPersistence, runs } = resolvePersistencePlan(persistence)
  if (!runs) {
