...
/**
 * Generation middleware entrypoint rules:
 * - `runs` is required (run lifecycle is the generation state this middleware
 *   always tracks)
 * - `artifacts` and `blobs` are optional, but come as a pair — the byte path
 *   writes the bytes to `blobs` and the describing row to `artifacts`, so one
 *   without the other is always a misconfiguration
 */
