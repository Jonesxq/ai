 ...
 * (which PROVIDES a durable lock via {@link withLocks}) and
 * `@tanstack/ai-sandbox` (whose `ensure` CONSUMES it to serialize resume-or-create)
 * import and re-export. Defining it here — core, depended on by both and
 * depending on neither — is what lets a persistence-provided distributed lock
 * reach `withSandbox` without either package depending on the other.
 */
...
/**
 * The lock capability. Provided by `withLocks` (from `@tanstack/ai-persistence`)
 * or any middleware that calls {@link provideLocks}.
 */
