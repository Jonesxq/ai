 ...
 * (whose `withPersistence` PROVIDES a durable store when `stores.sandbox` is
 * present) and `@tanstack/ai-sandbox` (whose `withSandbox` CONSUMES it in
 * `ensure`) import and re-export. Defining it here — core, depended on by both
 * and depending on neither — lets a persistence-provided store reach the sandbox
 * layer without either package depending on the other (the same arrangement as
 * the `'locks'` token).
 */
